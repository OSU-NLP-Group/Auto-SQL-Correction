...
class Having(ASTNode):
    ...
    def __eq__(self, other):
        ...
        if other is None:
            return False
        if not isinstance(other, Having):
            return False
    ...
    def parse_sql(self, tokens, idx):
        assert tokens[idx] == 'having', "'having' not found"
        idx += 1
    ...
    def print_sql(self):
        return "having " + self.cond.print_sql()

    def print_sql_dict(self):
        subqs = []
        clause = "having " + self.cond.print_sql_dict(subqs)
        if len(subqs) > 0:
            c = {"clause": clause}
    ...
    def print_python(self):
        return "having({})".format(self.cond.print_python())